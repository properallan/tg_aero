...
\begin{abstract}
O projeto aerotermodinâmico de sistemas de propulsão, como bocais de foguete, apresenta um desafio computacional crítico. Simulações de alta fidelidade (\textit{high-fidelity} - HF), baseadas nas equações de \textit{Reynolds-Averaged Navier-Stokes} (RANS) acopladas a modelos de transferência de calor conjugado (\textit{Conjugate Heat Transfer} - CHT), fornecem previsões precisas do fluxo de calor na parede, mas são proibitivamente caras para aplicações envolvendo múltiplas consultas, como otimização e quantificação de incerteza (\textit{uncertainty quantification} - UQ). Por outro lado, modelos de baixa fidelidade (\textit{low-fidelity} - LF), como as equaçoes de Euler quase unidimensionais, são computacionalmente baratos, mas incapazes de prever fenômenos viscosos e térmicos. Esta dissertação aborda esse dilema ao desenvolver e validar uma estrutura (\textit{framework}) multi-fidelidade orientada a dados para a reconstrução rápida e precisa de campos de escoamento bidimensionais, compressíveis e viscosos em um bocal convergente-divergente refrigerado a água. A metodologia utiliza um pequeno banco de dados pareado de simulações LF e HF para treinar um modelo substituto (surrogate) de baixo custo computacional. A Decomposição Ortogonal Própria (\textit{Proper Orthogonal Decomposition} - POD) é empregada para a redução de dimensionalidade, demonstrando eficiência excepcional ao comprimir os dados de alta dimensão em um espaço latente de apenas 3 componentes principais com perda de informação desprezível. A contribuição central deste trabalho é a comparação de duas técnicas de modelagem: a Rede Neural Artificial (\textit{Artificial Neural Network} - ANN) e o interpolador Kriging. Os resultados demonstram o sucesso da estrutura, com o Modelo de Ordem Reduzida (\textit{Reduced Order Model} - ROM) baseado em Kriging alcançando uma aceleração computacional de mais de 38.000 vezes em comparação com a simulação de alta fidelidade, mantendo uma precisão notável, evidenciada por um erro médio quadrático normalizado (\textit{normalized root mean squared error} - NRMSE) de apenas $1.89\times10^{-6}$ \% para o campo de Temperatura e $3.07\times10^{-4}$ \% para o fluxo de calor na parede. Um achado fundamental é o desempenho do modelo de Kriging em relação à ANN, nesse cenário específico, destacando que para problemas de engenharia caracterizados por mapeamentos suaves, de baixa dimensão e com dados limitados, métodos estatísticos tradicionais podem oferecer grande precisão e eficiência, até mesmo superando recentes arquiteturas de aprendizado de máquina. Este trabalho fornece uma estrutura robusta que quebra o compromisso entre custo computacional e precisão, permitindo análises de múltiplas consultas antes intratáveis, essenciais para a engenharia aeroespacial.
\end{abstract}
...
\vspace{0.5cm} % espaço entre as subfiguras
